...
namespace EOEPCA {
    ...
    public:
        ...
        explicit EOEPCAargo(const std::string &path) : mods::IModInterface(path), create_workflow_yaml(nullptr), create_workflow_yaml_from_app(nullptr) {
            if (isValid()) {
                ...
                std::cerr<<"creating create_workflow_yaml_from_app"<<std::endl;
                create_workflow_yaml_from_app = (void (*)(proc_comm_lib_argo::Application *, std::string &)) dlsym(handle, "create_workflow_yaml_from_app");
                if(!create_workflow_yaml_from_app){
                    std::cerr<<"create_workflow_yaml_from_app not found"<<std::endl;
                    throw std::runtime_error("create_workflow_yaml_from_app not found");
                }
                ...
                get_workflow_results_from_name = (void (*)(std::string_view, std::string_view, std::list<std::pair<std::string, std::string>> &, std::string_view, std::string_view)) dlsym(handle, "get_workflow_results_from_name");
    ...
    public:
        ...
        void (*get_workflow_results_from_name)(std::string_view, std::string_view, std::list<std::pair<std::string, std::string>> &, std::string_view, std::string_view);
    };
