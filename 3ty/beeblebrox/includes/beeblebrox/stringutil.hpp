...
#ifndef BEEBLEBROX__COMMON_STRING_UTIL_HPP
#define BEEBLEBROX__COMMON_STRING_UTIL_HPP
...
class StringUtil {
 public:
  ...
  // 						 std::not1(std::ptr_fun<int,
  // int>(std::isspace))).base(), s.end()); 	return s;
  // }
  ...
  //		int size = ((int) fmt.size()) * 2 + 50; // Use a rubric
  // appropriate for your code 		std::string str;
  // va_list ap; 		while (1) { // Maximum two passes on a POSIX
  // system... 			str.resize(size); 			va_start(ap,
  // fmt); int n = vsnprintf((char *) str.data(), size, fmt.c_str(), ap);
  // va_end(ap); 			if (n > -1 && n < size) { // Everything
  // worked 				str.resize(n);
  // return str;
  //			}
